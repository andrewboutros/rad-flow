...
// Although only the necessary ones are needed to be exported, we export all the
// packages at once here.
export const {
  mxGraph,
  mxCodec,
  mxConstants,
...
mxCodec.prototype.decode = function (node, into) {
  ...
  if (node && node.nodeType == mxConstants.NODETYPE_ELEMENT) {
    ...
    try {
      ...
      // @ts-ignore // since the `XML Node` to be processed may be not at `window`
