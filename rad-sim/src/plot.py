...
num_npu_tiles = 7
num_npu_blocks = num_npu_tiles + 5
...
with open('npu_traces') as traces_file:
    for line in traces_file:
        ...
        trace_height = vspacing * (num_npu_blocks - module_count)
...
tick_vals = []
tick_text = []
for i in range(num_npu_blocks):
  tick_vals.append(vspacing * (i+1))
for i in range(num_npu_tiles):
  tick_text.append('Tile ' + str(i))
tick_text.append('Accum')
tick_text.append('eVRF')
tick_text.append('MFU 0')
tick_text.append('MFU 1')
...
fig.update_layout(height=num_npu_blocks*70)
