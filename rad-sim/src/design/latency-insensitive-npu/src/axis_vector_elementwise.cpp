...
axis_vector_elementwise::axis_vector_elementwise(const sc_module_name& name, unsigned int inst_interface_dataw,
                                                 unsigned int wb_interface_dataw, unsigned int ififo_interface_dataw,
                                                 unsigned int id)
    : sc_module(name),
      evrf_inst_interfaces("evrf_inst_interface", SECTORS),
      mfu0_inst_interfaces("mfu0_inst_interface", SECTORS),
      mfu1_inst_interfaces("mfu1_inst_interface", SECTORS),
      loader_inst_interface("loader_inst_interface"),
      evrf_ififo_interfaces("evrf_ififo_interface", SECTORS),
      loader_wb0_interface("loader_wb0_interface"),
  ...
  for (unsigned int sector_id = 0; sector_id < SECTORS; sector_id++) {
    ...
    std::strcpy(module_name, module_name_str.c_str());
    evrf_modules[sector_id] = new evrf(module_name, sector_id, thread_id);
    ...
    evrf_inst_axis_interfaces[sector_id] = new axis_slave_fifo_adapter<evrf_mop, sc_bv<EVRF_MOP_BITWIDTH>>(
        module_name, inst, inst_interface_dataw, 1, EVRF_MOP_BITWIDTH, 1);
    ...
    std::strcpy(module_name, module_name_str.c_str());
    mfu0_modules[sector_id] = new mfu(module_name, 0, sector_id, thread_id);
    ...
    mfu0_inst_axis_interfaces[sector_id] = new axis_slave_fifo_adapter<mfu_mop, sc_bv<MFU_MOP_BITWIDTH>>(
        module_name, inst, inst_interface_dataw, 1, MFU_MOP_BITWIDTH, 1);
    ...
    std::strcpy(module_name, module_name_str.c_str());
    mfu1_modules[sector_id] = new mfu(module_name, 1, sector_id, thread_id);
  ...
  ld_module = new loader("ld", thread_id);
  ld_module->clk(clk);
  ld_module->rst(rst);
...
#ifdef RADSIM
  dest_node = module_assignment["axis_mvu_sector_0"];
#else
  dest_node = SECTOR_DEST_ID;
#endif
  ld_wb0_axis_interface = new axis_master_fifo_adapter<data_vector<tb_input_precision>, sc_bv<WIDE_WRITEBACK_BV_WIDTH>>(
      "ld_wb0_axis_interface", writeback_wide, wb_interface_dataw, CORES, LOW_PRECISION, dest_node,
