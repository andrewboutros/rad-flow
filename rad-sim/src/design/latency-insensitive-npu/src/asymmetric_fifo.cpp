...
asymmetric_fifo<dtype>::asymmetric_fifo(const sc_module_name& name, int depth, int iwidth, int owidth,
                                        int almost_full_size, int almost_empty_size) : sc_module(name) {
  // Setting input/output widths of the asymmetric FIFO (making sure input width is integer multiple of output width)
  if (iwidth % owidth != 0)
    npu_log.log(error, "Input width of asymmetric FIFO must be a multiple of its output width!", this->name());
  input_width = iwidth;
  output_width = owidth;
...
void asymmetric_fifo<dtype>::Tick() {
  ...
  while (true) {
    ...
    if (ren.read()) {
      if (mem.size() == 0)
        npu_log.log(error, "FIFO is underflowing!", this->name());
      mem.pop();
    }
    ...
    if (wen.read()) {
      if (mem.size() > (capacity - width_ratio)) 
        npu_log.log(error, "FIFO is overflowing!", this->name());
