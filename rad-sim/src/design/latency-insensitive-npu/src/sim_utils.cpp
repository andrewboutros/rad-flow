...
int GetNPUSimCycle(double period) {
  ...
  int cycle = (int)ceil(double(t.value()) / period / 1000);
  return cycle;
}
...
void init_vector<dtype>::init_sc_vector(sc_vector<std::vector<dtype>>& vector, unsigned int dim0, unsigned int dim1) {
  vector.init(dim0, [&](const char* nm, size_t) { return new std::vector<dtype>(dim1); });
}
...
sim_log::sim_log(unsigned int verbosity_level, std::string log_filename) {
  verbosity = verbosity_level;
  std::string log_filepath = NPU_ROOTDIR + log_filename;
  log_file.open(log_filepath);
}

sim_log::~sim_log() { log_file.close(); }

void sim_log::log(debug_t, std::string msg, sc_module_name module, bool log_to_file) {
  if (verbosity >= 4) {
...
void sim_log::log(info_t, std::string msg, sc_module_name module, bool log_to_file) {
  if (verbosity >= 3) {
...
void sim_log::log(trace_t, std::string msg, sc_module_name module, bool log_to_file) {
  if (verbosity >= 2) {
    std::cout << "\033[36m[TRACE] " << module << " @ " << GetNPUSimCycle(NPU_CLOCK_PERIOD) << ": " << msg << "\033[0m"
              << std::endl;
    if (log_to_file) {
      log_file << "[TRACE] " << module << " @ " << GetNPUSimCycle(NPU_CLOCK_PERIOD) << ": " << msg << std::endl;
    }
  }
}

void sim_log::log(warning_t, std::string msg, sc_module_name module, bool log_to_file) {
  if (verbosity >= 1) {
...
void sim_log::log(critical_t, std::string msg, sc_module_name module, bool log_to_file) {
  if (verbosity >= 0) {
...
void sim_log::log(error_t, std::string msg, sc_module_name module, bool log_to_file) {
  if (verbosity >= 0) {
...
sim_trace_probe::sim_trace_probe(std::string filename, unsigned int num_traces, unsigned int num_modules) {
  num_traces_monitored = num_traces;
  num_modules_monitored = num_modules;
  trace_filename = NPU_ROOTDIR + filename;
...
sim_trace_probe::~sim_trace_probe() {}

void sim_trace_probe::record_event(unsigned int module_id, unsigned int trace_id) {
  trace_cycles[trace_id][module_id].push_back(GetNPUSimCycle(NPU_CLOCK_PERIOD));
}

void sim_trace_probe::dump_traces() {
...
sim_log npu_log(VERBOSITY_LEVEL, "npu_log");
sim_trace_probe npu_trace_probe("npu_traces", NUM_TRACES, NUM_TRACE_MODULES);