...
axis_mvu_sector::axis_mvu_sector(const sc_module_name& name, unsigned int id, unsigned int inst_interface_dataw,
    ...
    : sc_module(name),
      ...
      sector_wb_interface("sector_wb_interface", THREADS),
  ...
  for (unsigned int thread_id = 0; thread_id < THREADS; thread_id++) {
    ...
    wb_interface_name_str = "sector" + std::to_string(sector_id) + "_wb_interface_" + std::to_string(thread_id);
    ...
    wb_axis_interface[thread_id] =
        new axis_slave_fifo_adapter<data_vector<tb_input_precision>, sc_bv<WIDE_WRITEBACK_BV_WIDTH>>(
            wb_interface_name, writeback_wide, wb_interface_dataw, CORES, LOW_PRECISION, LANES);
