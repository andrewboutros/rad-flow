...
void fifo<dtype>::Tick() {
  ...
  while (true) {
    ...
    if (ren.read()) {
      if (mem.size() == 0) npu_log.log(error, "FIFO is underflowing!", this->name());
      mem.pop();
    }

    // Push data into the FIFO if there is enough space
    if (wen.read()) {
      if (mem.size() == capacity) npu_log.log(error, "FIFO is overflowing!", this->name());
      std::vector<dtype> temp(dwidth);
      data_vector<dtype> wdata_temp = wdata.read();
      for (unsigned int element_id = 0; element_id < dwidth; element_id++) temp[element_id] = wdata_temp[element_id];
      mem.push(temp);
    }
