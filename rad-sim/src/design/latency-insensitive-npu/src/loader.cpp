...
loader::loader(const sc_module_name& name, unsigned int id)
    : sc_module(name),
...
void pack_data_and_dest_info_ch1(sc_signal<ld_uop>& ld_inst, data_vector<tb_output_precision>& write_back_data_vectors,
  ...
  for (unsigned int sector_id = 0; sector_id < SECTORS; sector_id++) {
    ...
    unsigned int dest_node, interface_id;
#ifdef RADSIM
    /*if (ld_inst.read().block1_id == 1) {
      dest_name = "axis_evrf_" + std::to_string(thread_id) + "_" + std::to_string(sector_id);
    ...
    dest_node = module_assignment[dest_name];
#else
    dest_node = (sector_id * 6) + ld_inst.read().block1_id;
#endif
    write_back_temp[3] = dest_node;
  ...
  while (true) {
    ...
    for (unsigned int core_id = 0; core_id < CORES; core_id++) {
      ...
      if (inst_valid_pipeline[LD_PIPELINE - 2].read() && inst_pipeline[LD_PIPELINE - 2].read().vrf1_id != 0 &&
        ...
        pack_data_and_dest_info_ch1(inst_pipeline[LD_PIPELINE - 2], data_from_pipeline, write_back_vectors, thread_id);
        for (unsigned int sector_id = 0; sector_id < SECTORS; sector_id++)
...
#ifdef GEN_TRACES
    if (inst_fifo_ren.read()) npu_trace_probe.record_event(SECTORS * (NUM_PIPELINE_BLOCKS - 1), UOP_ISSUE_TRACE);
    if (ext_output_fifo_wen_pipeline[LD_PIPELINE - 1].read() || wb0_fifo_wen_pipeline[LD_PIPELINE - 1].read() ||
      ...
      npu_trace_probe.record_event(SECTORS * (NUM_PIPELINE_BLOCKS - 1), UOP_RETIRE_TRACE);
    if (inst_fifo_ren.read() && inst_fifo_rdata.read().first_uop)
      npu_trace_probe.record_event(SECTORS * (NUM_PIPELINE_BLOCKS - 1), FIRST_UOP_ISSUE_TRACE);
    if ((ext_output_fifo_wen_pipeline[LD_PIPELINE - 1].read() || wb0_fifo_wen_pipeline[LD_PIPELINE - 1].read() ||
         wb1_fifo_wen_pipeline[LD_PIPELINE - 1].read()) &&
        inst_pipeline[LD_PIPELINE - 1].read().last_uop)
      npu_trace_probe.record_event(SECTORS * (NUM_PIPELINE_BLOCKS - 1), LAST_UOP_RETIRE_TRACE);
#endif

    /*if (wb0_fifo_wen_pipeline[LD_PIPELINE - 1].read()) {
      cout << this->name() << ": " << wb0_fifo_wdata[0] << endl;
    }*/
    /*if (wb1_fifo_wen_pipeline[LD_PIPELINE - 1].read() && inst_pipeline[LD_PIPELINE - 1].read().src_sel == LD_FROM_WB)
    { count ++; cerr << "Loader wrote to Ch1 -- " << count << endl;
    }*/
    /*for (unsigned int sector_id = 0; sector_id < SECTORS; sector_id++){
      cout << !ififo_empty[sector_id][0].read() << " ";
    }
    cout << " -- " << !inst_fifo_empty.read() << endl;*/
    wait();
  }
}
