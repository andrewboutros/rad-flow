...
mvu_sector::mvu_sector(const sc_module_name& name, unsigned int id)
  ...
  for (unsigned int core_id = 0; core_id < CORES; core_id++) {
    ...
    for (unsigned int dpe_id = 0; dpe_id < DPES_PER_SECTOR; dpe_id++) {
      ...
      for (unsigned int tile_id = 0; tile_id < TILES; tile_id++) {
        ...
        dpes[core_id][dpe_id][tile_id] = new simplified_dpe(
  ...
  for (unsigned int dpe_id = 0; dpe_id < DPES_PER_SECTOR; dpe_id++) {
    ...
    for (unsigned int tile_id = 0; tile_id < TILES; tile_id++) {
      ...
      mrf_path = NPU_ROOTDIR + mrf_filename;
...
void mvu_sector::Tick() {
  ...
  while (true) {
      ...
      if (!wb_ififo_empty[thread_id][0].read() && wb_ififo_ren[thread_id][0].read()) {
        ...
        if (temp_data.size() != 0 && temp_data[3].range(0, 0) == 1) {
          if (current_tag[thread_id].read() == MAX_TAG - 1)
            npu_log.log(error, "Tag value is overflowing!", this->name());
          current_tag[thread_id].write(current_tag[thread_id].read() + 1);
          npu_log.log(sim_trace,
                      "Thread " + std::to_string(thread_id) + " tag is updated to " +
                          std::to_string(current_tag[thread_id].read() + 1),
                      this->name());
#ifdef GEN_TRACES
          npu_trace_probe.record_event(sector_id * (NUM_PIPELINE_BLOCKS - 1), TAG_UPDATE_TRACE);
#endif
        }
      }
    }
    ...
    if (inst_fifo_ren[thread_id].read())
      npu_trace_probe.record_event(sector_id * (NUM_PIPELINE_BLOCKS - 1), UOP_ISSUE_TRACE);
    if (ofifo_wen_signal[thread_id_pipeline[SECTOR_INST_PIPELINE - 2].read()].read())
      npu_trace_probe.record_event(sector_id * (NUM_PIPELINE_BLOCKS - 1), UOP_RETIRE_TRACE);
    if (inst_fifo_rdata[thread_id].read().first_uop && inst_fifo_ren[thread_id].read())
      npu_trace_probe.record_event(sector_id * (NUM_PIPELINE_BLOCKS - 1), FIRST_UOP_ISSUE_TRACE);
    if (inst_pipeline[SECTOR_INST_PIPELINE - 2].read().last_uop &&
      ...
      npu_trace_probe.record_event(sector_id * (NUM_PIPELINE_BLOCKS - 1), LAST_UOP_RETIRE_TRACE);
#endif

    /*if(ofifo_wen_signal && sector_id == 1) {
      count++;
      cout << this->name() << ": wrote to OFIFO --  " << count << endl;
    }*/
    /*if(inst_pipeline[SECTOR_INST_PIPELINE - 1].read().last_uop) {
      cout << "Last uOP flag: " << GetNPUSimCycle(NPU_CLOCK_PERIOD) << endl;
      //count++;
      //cout << "Sector " << sector_id << " wrote to OFIFO --  " << count << endl;
    }*/
    /*for (unsigned int tile_id = 0; tile_id < TILES; tile_id++) {
      if (vrf_wen_signal[0][tile_id].read()) {
        cout << this->name() << ": " << vrf_wdata_signal[0][tile_id] << endl;
        //cin.get();
      }
    }*/
    wait();
