...
evrf::evrf(const sc_module_name& name, unsigned int id_sector, unsigned int id_thread)
    : sc_module(name),
      ...
      vrf_waddr_signal("vrf_waddr_signal", CORES),
...
void evrf::Tick() {
  ...
  while (true) {
...
#ifdef GEN_TRACES
        npu_trace_probe.record_event((sector_id * (NUM_PIPELINE_BLOCKS - 1)) + 1, TAG_UPDATE_TRACE);
#endif
      }
    }
    ...
    if (inst_fifo_ren.read())
      npu_trace_probe.record_event((sector_id * (NUM_PIPELINE_BLOCKS - 1)) + 1, UOP_ISSUE_TRACE);
    if (vrf_to_ofifo_valid_pipeline[EVRF_RF_TO_OFIFO_PIPELINE - 1][0].read())
      npu_trace_probe.record_event((sector_id * (NUM_PIPELINE_BLOCKS - 1)) + 1, UOP_RETIRE_TRACE);
    if (inst_fifo_ren.read() && inst_fifo_rdata.read().first_uop)
      npu_trace_probe.record_event((sector_id * (NUM_PIPELINE_BLOCKS - 1)) + 1, FIRST_UOP_ISSUE_TRACE);
    if (vrf_to_ofifo_valid_pipeline[EVRF_RF_TO_OFIFO_PIPELINE - 1][0].read() &&
        inst_pipeline[EVRF_INST_PIPELINE - 1].read().last_uop)
      npu_trace_probe.record_event((sector_id * (NUM_PIPELINE_BLOCKS - 1)) + 1, LAST_UOP_RETIRE_TRACE);
#endif

    // if (vrf_to_ofifo_valid_pipeline[EVRF_RF_TO_OFIFO_PIPELINE - 1][0] && sector_id == 2) {
    //  count++;
    //  cout << this->name() << " wrote to OFIFO -- " << count << endl;
    //}
    /*if (vrf_to_ofifo_valid_pipeline[EVRF_RF_TO_OFIFO_PIPELINE - 1][0] && sector_id == 0) {
      count++;
      cout << this->name() << " word is wrote to OFIFO -- " << count << endl;
    }*/
    /*if (sector_id == 0 && !ififo_empty[0].read()) {
      cout << "Inst rdy: " << !inst_fifo_empty.read() << ", current tag: " << current_tag << ", Inst tag: " <<
        inst_fifo_rdata.read().tag << ", src: " << inst_fifo_rdata.read().src_sel << ", IFIFO rdy: " <<
        !ififo_empty[0].read() << endl;
    }*/
    wait();
  }
}
