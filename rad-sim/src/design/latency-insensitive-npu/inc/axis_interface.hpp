...
struct axis_master_port {
  ...
  axis_master_port() : tvalid(), tready(), tdata(), tstrb(), tkeep(), tlast(), tid(), tdest(), tuser() {}
  axis_master_port(const axis_master_port& m) {}
...
struct axis_slave_port {
  ...
  axis_slave_port() : tvalid(), tready(), tdata(), tstrb(), tkeep(), tlast(), tid(), tdest(), tuser() {}
  axis_slave_port(const axis_slave_port& s) {}
  axis_slave_port(const sc_module_name& name)
      : tvalid(), tready(), tdata(), tstrb(), tkeep(), tlast(), tid(), tdest(), tuser() {}

  // Helper function for reseting all output ports of an AXI-stream master interface
  void Reset() { tready.write(false); }
...
struct axis_signal {
  ...
  axis_signal() : tvalid(), tready(), tdata(), tstrb(), tkeep(), tlast(), tid(), tdest(), tuser() {}
  ...
  void Connect(axis_master_port& m) {
    // Connect signal to master port
    m.tvalid(tvalid);
    m.tready(tready);
    m.tdata(tdata);
