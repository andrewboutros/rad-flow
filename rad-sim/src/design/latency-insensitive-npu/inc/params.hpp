...
#define NPU_ROOTDIR "/Users/andrew/PhD/dev/rad-sim-opt-npu-multithread-hard-c2/src/design/latency-insensitive-npu/"
...
#define SECTOR_INST_TO_ACCUM_PIPELINE \
  (SECTOR_INST_TO_DPES_PIPELINE + SECTOR_DPES_PIPELINE + SECTOR_REDUCTION_PIPELINE + SECTOR_REDUCTION_TO_ACCUM_PIPELINE)
#define ACCUM_PIPELINE (RF_RD_LATENCY + 1)
#define SECTOR_INST_PIPELINE                                                         \
  (SECTOR_INST_TO_DPES_PIPELINE + SECTOR_DPES_PIPELINE + SECTOR_REDUCTION_PIPELINE + \
   SECTOR_REDUCTION_TO_ACCUM_PIPELINE + ACCUM_PIPELINE + SECTOR_ACCUM_TO_OFIFO_PIPELINE)
...
#define NARROW_WRITEBACK_BV_WIDTH \
  ((CORES * DPES_PER_SECTOR * HIGH_PRECISION) + VRF_WB_SELW + BLOCK_WB_SELW + VRF_ADDRW + 1)
...
#define AXIS_TRANSACTION_WIDTH (AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + AXIS_USERW + AXIS_DESTW + AXIS_IDW + 1)
#endif
...
#define TUSER(bv) \
  (bv.range(AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + AXIS_USERW - 1, AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW))
#define TUSER_FLAG(bv) (bv.range(AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW, AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW))
#define TUSER_ADDR(bv) \
  (bv.range(AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + VRF_ADDRW, AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + 1))
#define TUSER_VRFID(bv)                                            \
  (bv.range(AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + AXIS_USERW - 1, \
            AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + VRF_ADDRW + 1))
#define TDEST(bv)                                                               \
  (bv.range(AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + AXIS_USERW + AXIS_DESTW - 1, \
           AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + AXIS_USERW))
#define TID(bv)                                                                            \
  (bv.range(AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + AXIS_USERW + AXIS_DESTW + AXIS_IDW - 1, \
            AXIS_MAX_DATAW + AXIS_STRBW + AXIS_KEEPW + AXIS_USERW + AXIS_DESTW))
#define TLAST(bv) (bv.range(AXIS_TRANSACTION_WIDTH - 1, AXIS_TRANSACTION_WIDTH - 1))

// Destinations & stream IDs
#define SECTOR_DEST_ID 0
#define EVRF_DEST_ID 1
#define MFU0_DEST_ID 2
#define MFU1_DEST_ID 3
#define LD_DEST_ID 4
#define SECTOR_INST_STREAM_ID 0
#define SECTOR_WB_STREAM_ID THREADS
#define EVRF_INST_STREAM_ID 0
#define EVRF_WB_STREAM_ID 1
#define EVRF_IN_STREAM_ID 1
#define MFU_INST_STREAM_ID 2
#define MFU_WB_STREAM_ID 1
#define MFU_IN_STREAM_ID 2
#define LD_INST_STREAM_ID 0
#define LD_IN_STREAM_ID 0

// Trace probe defines
#define GEN_TRACES 1
#define NUM_TRACES 5
#define NUM_TRACE_MODULES ((SECTORS * (NUM_PIPELINE_BLOCKS - 1)) + 1)
