...
class axis_slave_adapter : public sc_module {
 private:
  int _node_id;  // Node ID of this adapter
  float _node_period;
  int _network_id;
  int _num_axis_interfaces;
  std::vector<Flit::FlitType> _interface_types;
  std::vector<unsigned int> _interface_dataw;
  std::vector<int> _num_flits_per_packet;
  int _max_num_flits_per_packet;
  int _freq_ratio;
  ...
  sc_signal<int> _input_axis_transaction_id;
  sc_signal<Flit::FlitType> _input_axis_transaction_type;
  sc_signal<int> _input_axis_transaction_interface;
 ...
 public:
  ...
  axis_slave_adapter(const sc_module_name& name, int node_id, int network_id,
                     ...
                     float node_period, BookSimConfig* noc_config, Network* noc, BufferState* buffer_state,
