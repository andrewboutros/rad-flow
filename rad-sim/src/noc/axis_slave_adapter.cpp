...
axis_slave_adapter::axis_slave_adapter(const sc_module_name &name, int node_id, int network_id,
                                       std::vector<Flit::FlitType> &interface_types,
                                       std::vector<unsigned int> &interface_dataw, float node_period,
                                       BookSimConfig *noc_config, Network *noc, BufferState *buffer_state,
                                       tRoutingFunction routing_func, bool lookahead_routing, bool wait_for_tail_credit)
    : sc_module(name) {
  ...
  _freq_ratio = (int) ceil(_node_period * 1.0 / ADAPTER_PERIOD);
  for (unsigned int interface_id = 0; interface_id < _num_axis_interfaces; interface_id++) {
    int payload_dataw = AXIS_TRANSACTION_PAYLOAD_WIDTH - AXIS_MAX_DATAW + interface_dataw[interface_id];
    _num_flits_per_packet[interface_id] = (int)ceil(payload_dataw * 1.0 / NOC_LINKS_PAYLOAD_WIDTH);
    _max_num_flits_per_packet = std::max(_max_num_flits_per_packet, _num_flits_per_packet[interface_id]);
  }
  ...
  _injection_afifo_depth = RADSimConfig::GetIntKnob("noc_adapter_fifo_depth");
...
int axis_slave_adapter::GetInputDestinationNode(const sc_bv<AXIS_DESTW> axis_transaction_dest) {
...
void axis_slave_adapter::InputReady() {
  ...
  // If packetization is busy, skip arbitration logic since adapter can't accept new transaction anyway
  if (!_packetization_busy.read()) {
    // Go over interfaces (in order of their current priority) -- if the injection FIFO is not full, the first interface
    // with valid data is going to be accepted and its corresponding tready value is set to true
    for (unsigned int interface_id = 0; interface_id < _num_axis_interfaces; interface_id++) {
  ...
  // If no transation is registered, update interface priority in a round robin fashion
...
// Sequential logic for registering an AXI streaming transaction from one of the input interfaces -- this typically
// operates at the node frequency
void axis_slave_adapter::InputInterface() {
  ...
  while (true) {
    // Go over all interfaces checking for one with both tready and tvalid set to true. If found, register this AXI
    // streaming transaction for packetization
    bool transaction_registered = false;
    for (unsigned int interface_id = 0; interface_id < _num_axis_interfaces; interface_id++) {
      if (axis_interfaces[interface_id].tready.read() && axis_interfaces[interface_id].tvalid.read()) {
        ...
        _input_axis_transaction.tdata.write(axis_interfaces[interface_id].tdata.read());
        _input_axis_transaction.tdest.write(axis_interfaces[interface_id].tdest.read());
        _input_axis_transaction.tid.write(axis_interfaces[interface_id].tid.read());
        _input_axis_transaction.tkeep.write(axis_interfaces[interface_id].tkeep.read());
        _input_axis_transaction.tlast.write(axis_interfaces[interface_id].tlast.read());
        _input_axis_transaction.tstrb.write(axis_interfaces[interface_id].tstrb.read());
        _input_axis_transaction.tuser.write(axis_interfaces[interface_id].tuser.read());
        _input_axis_transaction.tvalid.write(axis_interfaces[interface_id].tvalid.read());
        transaction_registered = true;
    ...
    // If no transactions were registered, set the valid signal of the registered transaction to false
    if (!transaction_registered) _input_axis_transaction.tvalid.write(false);
    wait();
  }
}

void set_flit_payload(sc_flit &packetization_flit, sc_bv<AXIS_TRANSACTION_WIDTH> &packet_bv, int flit_id) {
  unsigned int start_idx = flit_id * NOC_LINKS_PAYLOAD_WIDTH;
  unsigned int end_idx = std::min((flit_id + 1) * NOC_LINKS_PAYLOAD_WIDTH, AXIS_TRANSACTION_PAYLOAD_WIDTH);
...
void axis_slave_adapter::InputPacketization() {
  ...
  while (true) {
    ...
    if (_input_axis_transaction.tvalid.read() || _packetization_busy.read()) {
      // During the first packetization cycle, form as many flits as needed by this transaction and push them to the
      // injection FIFO -- In hardware, this happens over P clock cycles where a flit is formed and buffered each cycle,
      // but is modeled by doing all the work in the 1st cycle & remaining idle for (P-1) cycles
      if (_packetization_cycle == 0) {
        ...
        for (int flit_id = 0; flit_id < _num_flits_per_packet[used_interface_id]; flit_id++) {
          sc_flit packetization_flit(
              flit_id == 0, flit_id == _num_flits_per_packet[used_interface_id] - 1, _input_axis_transaction_type,
              VCIDFromType(_input_axis_transaction_type, _noc_config), AXIS_TDEST(packet_bv).to_uint(),
              AXIS_TID(packet_bv).to_uint(), _input_axis_transaction_id.read(), _input_axis_transaction_id.read());
          set_flit_payload(packetization_flit, packet_bv, flit_id);
          _injection_afifo.push(packetization_flit);
        }
        _packetization_cycle += 1;

        // If number of flits to be formed needs more than 1 node clock period, set the packetization busy flag to be
        // true such that the adapter does not accept a new transaction before finishing the packetization of the
        // previous one
        _packetization_busy.write(_num_flits_per_packet[used_interface_id] > _freq_ratio);
      } else if (_packetization_cycle == std::max(_num_flits_per_packet[used_interface_id], _freq_ratio) - 1) {
        _packetization_cycle = 0;
        _packetization_busy.write(false);
        NoCTransactionTelemetry::RecordTransactionTailPacketization(_input_axis_transaction_id.read());
      } else {
        _packetization_cycle += 1;
      } 
    }
    _injection_afifo_full.write(_injection_afifo.size() >= _injection_afifo_depth - _max_num_flits_per_packet);
    wait();
  }
}
...
void axis_slave_adapter::InputInjection() {
  ...
  while (true) {
    if (!_injection_afifo.empty() || _injection_flit_ready) {
      ...
      if (!_buffer_state->IsFullFor((int)_to_be_injected_flit._vc_id.to_uint()) &&
          (!_to_be_injected_flit._head || !_wait_for_tail_credit ||
           _buffer_state->IsAvailableFor((int)_to_be_injected_flit._vc_id.to_uint()))) {
        ...
        booksim_flit->ctime = GetSimulationCycle(NOC_PERIOD);
        booksim_flit->itime = GetSimulationCycle(NOC_PERIOD);
        booksim_flit->cl = 0;
        ...
        booksim_flit->dest = GetInputDestinationNode(_to_be_injected_flit._dest);
        booksim_flit->dest_interface = _to_be_injected_flit._dest_interface.to_uint();
        booksim_flit->pri = 0;
        ...
        _injection_flit_ready = false;
        if (booksim_flit->head) NoCTransactionTelemetry::RecordTransactionHeadInjection(booksim_flit->id);
      }
    }
