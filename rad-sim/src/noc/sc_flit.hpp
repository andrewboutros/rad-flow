...
class sc_flit {
 ...
 public:
  ...
  int _sim_transaction_id;                                // Unique simulation transaction ID for this flit

  sc_flit();
  sc_flit(const sc_flit& f);
  sc_flit(bool head, bool tail, Flit::FlitType type, int vc_id, const sc_uint<NOC_LINKS_DEST_WIDTH>& dest,
          const sc_bv<NOC_LINKS_DEST_INTERFACE_WIDTH>& dest_interface, const sc_bv<NOC_LINKS_PACKETID_WIDTH>& packet_id,
          int sim_transaction_id);
...
class sc_packet {
 private:
  ...
  int _stream_id;                // Stream ID of this packet (to choose right adapter interface)

 public:
  sc_packet();
  int GetNumValidFlits();
  sc_flit* GetFlit(int idx);
