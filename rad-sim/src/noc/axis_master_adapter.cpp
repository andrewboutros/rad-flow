...
axis_master_adapter::axis_master_adapter(const sc_module_name &name, int node_id, int network_id,
  ...
  _max_num_flits = (int)ceil(AXIS_TRANSACTION_PAYLOAD_WIDTH * 1.0 / NOC_LINKS_PAYLOAD_WIDTH);
  ...
  for (unsigned int interface_id = 0; interface_id < _num_axis_interfaces; interface_id++) {
    ...
    _num_flits[interface_id] = (int)ceil(payload_dataw * 1.0 / NOC_LINKS_PAYLOAD_WIDTH);;
  }
  _num_vcs = RADSimConfig::GetIntKnob("num_virtual_channels");
  ...
  _output_afifo_depth = RADSimConfig::GetIntKnob("noc_adapter_output_fifo_depth");
  ...
  while (true) {
    ...
    if (_ejected_booksim_flit) {
      ...
      if (_ejection_afifos[vc_id].size() < _ejection_afifo_depth) {
        ...
        sc_flit ejected_flit(_ejected_booksim_flit->head, _ejected_booksim_flit->tail, _ejected_booksim_flit->type,
                             _ejected_booksim_flit->vc, _ejected_booksim_flit->dest,
                             _ejected_booksim_flit->dest_interface, _ejected_booksim_flit->pid,
                             _ejected_booksim_flit->id);
        ejected_flit._payload = static_cast<sc_bv<NOC_LINKS_PAYLOAD_WIDTH> *>(_ejected_booksim_flit->data);
        ejected_flit._vc_id = vc_id;
        _ejection_afifos[vc_id].push(ejected_flit);

        // If tail flit received, increment FIFO counter (i.e. FIFO has 1 more complete packet)
        if (_ejected_booksim_flit->tail) {
          NoCTransactionTelemetry::RecordTransactionTailEjection(_ejected_booksim_flit->id);
          NoCTransactionTelemetry::UpdateHops(_ejected_booksim_flit->id, _ejected_booksim_flit->hops);
...
void axis_master_adapter::OutputDepacketization() {
  ...
  while (true) {
    ...
    if (!_ejection_afifos[highest_priority_fifo_id].empty()) {
      dest_output_fifo = _ejection_afifos[highest_priority_fifo_id].front()._dest_interface.to_uint();
    }
    ...
    // unsigned int dest_output_fifo = _ejection_afifos[highest_priority_fifo_id].front()._dest_interface.to_uint();
    // If output double buffer (FIFO of size 2) has free space, depacketize flits
    if (_output_afifos[dest_output_fifo].size() < _output_afifo_depth && fifo_has_complete_packet) {
      // Pop a flit from the highest priority ejection FIFO
      sc_flit processed_flit = _ejection_afifos[highest_priority_fifo_id].front();
      _ejection_afifos[highest_priority_fifo_id].pop();
      _constructed_packet.AddFlit(processed_flit);
      if (processed_flit._head) {
        sc_bv<NOC_LINKS_PAYLOAD_WIDTH> head_payload = *(processed_flit._payload);
      ...
      if (processed_flit._tail) {
        ...
        NoCTransactionTelemetry::RecordTransactionTailDepacketization(processed_flit._sim_transaction_id);
        _ejection_afifo_is_depacketizing.write(false);
...
void axis_master_adapter::write_sc_packet_to_axis_output(sc_packet &packet, axis_master_port &axis_port,
                                                         int output_chunk, unsigned int interface_id) {
  sc_bv<AXIS_TRANSACTION_WIDTH> packet_bv;
  sc_bv<AXIS_TRANSACTION_PAYLOAD_WIDTH> packet_payload_bv;
  int start_idx, end_idx;
  for (int flit_id = 0; flit_id < packet.GetNumValidFlits(); flit_id++) {
    if (flit_id == 0) {
      AXIS_TDEST(packet_bv) = packet.GetFlit(flit_id)->_dest;
    }
    start_idx = flit_id * NOC_LINKS_PAYLOAD_WIDTH;
    end_idx = std::min(AXIS_TRANSACTION_PAYLOAD_WIDTH, (flit_id + 1) * NOC_LINKS_PAYLOAD_WIDTH);
    packet_payload_bv.range(end_idx - 1, start_idx) =
        packet.GetFlit(flit_id)->_payload->range(NOC_LINKS_PAYLOAD_WIDTH - 1, 0);
  }
  ...
  AXIS_TDATA(packet_bv).range(AXIS_MAX_DATAW - 1, _interface_dataw[interface_id]) = 0;
  ...
  while (true) {
    for (unsigned int interface_id = 0; interface_id < _num_axis_interfaces; interface_id++) {
      if (axis_interfaces[interface_id].tready.read() && axis_interfaces[interface_id].tvalid.read()) {
        ...
        NoCTransactionTelemetry::RecordTransactionReceipt(output_packet.GetFlit(0)->_sim_transaction_id);
        _output_chunk[interface_id]++;

        int total_num_flits = output_packet.GetNumValidFlits();
        int num_chunks = (int)ceil(total_num_flits * 1.0 / _num_flits[interface_id]);
        if (_output_chunk[interface_id] == num_chunks) {
          for (unsigned int flit_id = 0; flit_id < output_packet.GetNumValidFlits(); flit_id++) {
      ...
      if (!_output_afifos[interface_id].empty()) {
        sc_packet output_packet = _output_afifos[interface_id].front();
        write_sc_packet_to_axis_output(output_packet, axis_interfaces[interface_id], _output_chunk[interface_id],
                                       interface_id);
      } else {
