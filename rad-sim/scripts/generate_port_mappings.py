...
def is_axis_port(port, module):
    if re.match(verilog_axis_regex.format(module), port.name):
        return True
    return False

def is_axis_role_master(module):
    for p in module.ports:
        if is_axis_port(p, module.name):
...
def generate_port_mappings_for_module(port_mapping_file, module):
    warnings = False
    port_mapping_file.write("module {0}\n".format(module.name))
    is_master = is_axis_role_master(module) # Scan all ports to determine whether AXI-S interface is master or slave
    for p in module.ports:
        if is_axis_port(p, module.name):
            signal = re.match(verilog_axis_regex.format(module.name), p.name)
    ...
    with open(port_map_file_path, "w") as port_mapping_file:
        ...
        for i in range(len(rtl_files)):
            ...
            if vlog.is_verilog(rtl_file_path):
                ...
                exit()
            for m in modules:
                warnings = True if generate_port_mappings_for_module(port_mapping_file, m) else warnings
        if warnings:
