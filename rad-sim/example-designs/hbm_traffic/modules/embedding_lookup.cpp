    ...
    for (unsigned int ctrl_id = 0; ctrl_id < _num_channels_per_ctrl.size();
      ...
      for (unsigned int c = 0; c < _num_channels_per_ctrl[ctrl_id]; c++) {
        if (!_lookup_indecies_fifo[ch_id].empty()) {
          ...
          std::stringstream ss;
          ss << "black_box_" << black_box_names[ch_id] << "_inst.aximm_interface";
          src_port_name = ss.str();
          // if (ch_id == 0) {
          //   src_port_name = "black_box_A_inst.aximm_interface";
          // } else {
          //   // src_port_name = "feature_interaction_inst.aximm_interface_" + std::to_string(ch_id);
          // }
...
void embedding_lookup::RegisterModuleInfo() {
  std::string port_name;
