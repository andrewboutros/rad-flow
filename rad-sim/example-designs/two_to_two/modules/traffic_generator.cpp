...
traffic_generator::traffic_generator(const sc_module_name &name, unsigned int fifo_depth, std::string inst_num)
    : RADSimModule(name) {
...
void traffic_generator::Tick() {
  ...
  while (true) {
    ...
    if (!req_wdata_fifo.empty()) {
      ...
      std::string dst_port_name = "ext_mem_" + inst_n + ".mem_channel_0";
      uint64_t dst_addr = radsim_design.GetPortBaseAddress(dst_port_name) +
                          req_addr_fifo.front();
      std::string src_port_name = "traffic_generator_inst_" + inst_n +".aximm_req_interface";
      uint64_t src_addr = radsim_design.GetPortBaseAddress(src_port_name);
      // std::cout << "dst add base: " << std::to_string(radsim_design.GetPortBaseAddress(dst_port_name))
      //           << " req fifo front: " <<  std::to_string(req_addr_fifo.front()) << " src dst: " << std::to_string(src_addr) << std::endl;
      if (type == 0) {
        aximm_req_interface.araddr.write(dst_addr);
        aximm_req_interface.arid.write(id_counter.read());
        aximm_req_interface.arlen.write(1);
        aximm_req_interface.arburst.write(0);
        aximm_req_interface.arsize.write(DATAW);
        ...
        aximm_req_interface.awlen.write(1);
        aximm_req_interface.awburst.write(0);
        aximm_req_interface.awsize.write(DATAW);
