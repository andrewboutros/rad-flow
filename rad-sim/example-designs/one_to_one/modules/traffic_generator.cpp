...
traffic_generator::traffic_generator(const sc_module_name &name, unsigned int fifo_depth)
    : RADSimModule(name) {
...
void traffic_generator::Tick() {
  ...
  while (true) {
    ...
    if (!req_wdata_fifo.empty()) {
      ...
      std::string dst_port_name = "ext_mem_0.mem_channel_0";
      uint64_t dst_addr = radsim_design.GetPortBaseAddress(dst_port_name) +
                          req_addr_fifo.front();
      std::string src_port_name = "traffic_generator_inst.aximm_req_interface";
      ...
      if (type == 0) {
        ...
        std::cout << "come on.... " << boost::lexical_cast<std::string>(aximm_req_interface.awlen.read()) << std::endl;
