...
dispatcher::dispatcher(const sc_module_name& name, unsigned int id) : 
  ...
  SC_METHOD(Assign);
  sensitive << rst << data_fifo_empty_signal << data_fifo_rdata_signal << data_fifo_almost_full_signal 
...
void dispatcher::Assign() {
  if (rst.read()) {
    ...
    if (tx_tdata.size() > 0) {
      ...
      //std::cout << "Dispatcher " << dispatcher_id << " pushed data into the NoC with dest " 
      //  << radsim_design.GetPortDestinationID(dest_name) << "!" << std::endl;
    } else {
      tx_interface.tvalid.write(false);
    }

    data_fifo_ren_signal.write(tx_interface.tvalid.read() && tx_interface.tready.read());
