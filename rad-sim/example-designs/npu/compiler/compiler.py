	...
	def launch_perf_sim(self, num_tiles, num_sectors, num_dpes, num_lanes, num_threads, vrf_depth, mrf_depth, perf_simulation, is_first, verbose = False):
		...
		if (is_first):
			...
			with open(dump_path, 'w') as defines:
				...
				defines.write('#define AXIS_TRANSACTION_WIDTH (AXIS_PAYLOADW + AXIS_STRBW + AXIS_KEEPW + AXIS_IDW + AXIS_DESTW)')
				defines.write('#define TUSER_FLAG(bv)  (AXIS_TUSER_RANGE(bv, 0, 0))\n')
				defines.write('#define TUSER_ADDR(bv)  (AXIS_TUSER_RANGE(bv, VRF_ADDRW, 1))\n')
				defines.write('#define TUSER_VRFID(bv) (AXIS_TUSER_RANGE(bv, AXIS_USERW - 1, VRF_ADDRW + 1))\n')
