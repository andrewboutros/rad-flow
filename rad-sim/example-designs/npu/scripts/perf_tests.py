...
for workload in workloads:
	...
	if (first_flag):
		subprocess.call(['python', workload+'.py', '-perfsim', '-is_first'], stdout=outfile, stderr=outfile, shell=False)
		first_flag = False
	else:
		subprocess.call(['python', workload+'.py', '-perfsim'], stdout=outfile, stderr=outfile, shell=False)
			...
			if('PASSED' in args[3]):
				...
				if workload in baseline_results:
					...
					if comparison_to_baseline >= 0:
						print ('{:<5}    +{:<5.2f}%    {:<8}    '.format(result, comparison_to_baseline, int(cycles)), end='')
					else:
						print ('{:<5}     {:<5.2f}%    {:<8}    '.format(result, comparison_to_baseline, int(cycles)), end='')
				else:
					print ('{:<5}    {:<7}    {:<8}    '.format(result, 'N/A', int(cycles)), end='')
			else:
