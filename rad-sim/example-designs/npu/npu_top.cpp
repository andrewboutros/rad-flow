...
npu_top::npu_top(const sc_module_name &name) : sc_module(name),
  ...
  first_mvu_sector = new axis_mvu_sector("axis_mvu_sector_0", 0);
  ...
  for (unsigned int sector_id = 1; sector_id < SECTORS; sector_id++) {
    ...
    mvu_sectors[sector_id - 1] = new axis_mvu_sector_chain(module_name, sector_id);
  ...
  for (unsigned int thread_id = 0; thread_id < THREADS; thread_id++) {
    ...
    std::strcpy(module_name, module_name_str.c_str());
    inst_dispatcher[thread_id] = new axis_inst_dispatch(module_name, thread_id);
    ...
    std::strcpy(module_name, module_name_str.c_str());
    vector_elementwise_blocks[thread_id] = new axis_vector_elementwise(module_name, thread_id);
