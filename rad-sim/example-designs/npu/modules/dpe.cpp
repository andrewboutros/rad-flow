...
dpe::dpe(const sc_module_name& name, unsigned int id, unsigned int tile_id) :
        sc_module(name),
...
dpe::~dpe() { 
  for( unsigned int t = 0; t < 2; t++ ) {
    for (unsigned int dot_id = 0; dot_id < TB_NUM_DOTS; dot_id++) tb_regs[t][dot_id].clear();
  }
}

void dpe::AdvanceControlPipelines(){
    // Reset logic
