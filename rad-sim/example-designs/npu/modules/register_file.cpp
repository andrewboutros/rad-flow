...
register_file<dtype>::register_file(const sc_module_name& name, unsigned int id, unsigned int depth, unsigned int width,
  ...
  if (!init_file.empty()) {
    bool parse = parse_register_file_contents_from_file(mem, init_file, width, depth);
    if (!parse) {
  ...
  SC_METHOD(Assign);
  sensitive << wdata_pipeline[RF_WR_LATENCY - 1] << wen_pipeline[RF_WR_LATENCY - 1] << waddr_pipeline[RF_WR_LATENCY - 1]
...
template <class dtype>
register_file<dtype>::~register_file() {
  for (unsigned int i = 0; i < mem.size(); i++) mem[i].clear();
}

template <class dtype>
void register_file<dtype>::Assign() {
  // Assign read data output
  rdata.write(data_vector<dtype>(mem[raddr_pipeline[RF_RD_LATENCY - 1].read()]));
  ...
  if (wen_pipeline[RF_WR_LATENCY - 1].read()) {
    ...
    if (temp.size() != 0)
      for (unsigned int i = 0; i < num_elements_per_word; i++) mem[addr][i] = temp[i];
  }
}

template <class dtype>
void register_file<dtype>::Tick() {
...
template class register_file<tb_input_precision>;
template class register_file<tb_output_precision>;