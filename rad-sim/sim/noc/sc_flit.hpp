...
// SystemC flit class to encapsulate different flit components. Flit payload
// pointers are recycled when a flit is no longer in use (i.e., freed) to avoid
// creating new pointers throughout the simulation time
class sc_flit {
private:
  static std::stack<sc_bv<NOC_LINKS_PAYLOAD_WIDTH> *>
      _all; // Stack of all payload pointers
  static std::stack<sc_bv<NOC_LINKS_PAYLOAD_WIDTH> *>
      _free; // Stack of recycled free payload pointers
...
private:
  ...
  unsigned int _stream_id; // Stream ID of this packet (to choose right adapter
                           // interface)
