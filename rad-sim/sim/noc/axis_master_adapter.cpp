...
axis_master_adapter::axis_master_adapter(const sc_module_name &name, int node_id, int network_id,
  ...
  _ejection_afifo_depth = radsim_config.GetIntVectorKnob("adapter_fifo_size", _network_id);
  ...
  _output_afifo_depth = radsim_config.GetIntVectorKnob("adapter_obuff_size", _network_id);
...
void axis_master_adapter::write_sc_packet_to_axis_output(sc_packet &packet, axis_master_port &axis_port,
  ...
  for (unsigned int flit_id = 0; flit_id < packet.GetNumValidFlits(); flit_id++) {
    if (flit_id == 0) {
      AXIS_TDEST(packet_bv) = packet.GetFlit(flit_id)->_dest;
    }
    start_idx = flit_id * NOC_LINKS_PAYLOAD_WIDTH;
    end_idx = std::min(AXIS_TRANSACTION_PAYLOAD_WIDTH, (int) ((flit_id + 1) * NOC_LINKS_PAYLOAD_WIDTH));
