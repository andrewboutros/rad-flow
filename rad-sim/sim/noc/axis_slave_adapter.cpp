...
axis_slave_adapter::axis_slave_adapter(const sc_module_name &name, int node_id, int network_id,
  ...
  for (unsigned int interface_id = 0; interface_id < _num_axis_interfaces; interface_id++) {
    ...
    _max_num_flits_per_packet = std::max((int)_max_num_flits_per_packet, (int)_num_flits_per_packet[interface_id]);
  }
  ...
  _injection_afifo_depth = radsim_config.GetIntVectorKnob("adapter_fifo_size", _network_id);
  ...
  while (true) {
    ...
    if (_input_axis_transaction.tvalid.read() || _packetization_busy.read()) {
      ...
      if (_packetization_cycle == 0) {
        ...
        for (unsigned int flit_id = 0; flit_id < _num_flits_per_packet[used_interface_id]; flit_id++) {
      ...
      } else if (_packetization_cycle == std::max((int)_num_flits_per_packet[used_interface_id], (int)_freq_ratio) - 1) {
        _packetization_cycle = 0;
        _packetization_busy.write(false);
