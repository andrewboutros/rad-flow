...
class mem_controller : public sc_module {
 private:
  // The DRAMsim memory system model
  dramsim3::MemorySystem* _dramsim;
  // Number of memory channels and interfaces of the controller (specified by DRAMsim)
  ...
  // The ratio between these clock periods has to match that of the interface/channel bitwidths for rate matching
  ...
  // Book-keeping of memory contents for each channel (hashtable of address -> data)
  ...
  std::vector<std::queue<std::pair<uint64_t, sc_bv<AXI_MAX_DATAW>>>> _write_data_queue;
  std::vector<std::queue<std::tuple<uint64_t, uint64_t, bool>>> _read_address_queue;
  // Queue for requests already dispatched to DRAMsim but waiting for responses (assuming in-order)
  // For each channel, keep a queue of response address (and data if write request)
  std::vector<std::queue<std::tuple<uint64_t, sc_bv<AXI_MAX_DATAW>, bool>>> _outstanding_write_requests;
  std::vector<std::queue<std::pair<uint64_t, bool>>> _outstanding_read_requests;
  // Queue for responses populated by read/write callback functions
  std::vector<std::queue<std::tuple<uint64_t, sc_bv<AXI_MAX_DATAW>, bool>>> _output_read_responses;
  ...
  sc_vector<sc_signal<unsigned int>> _write_address_queue_occupancy, _write_data_queue_occupancy, _read_address_queue_occupancy;
  sc_vector<sc_signal<unsigned int>> _num_outstanding_write_requests, _num_outstanding_read_requests;
  sc_vector<sc_signal<unsigned int>> _output_write_queue_occupancy, _output_read_queue_occupancy;
  ...
  unsigned int _num_ranks, _num_bank_groups, _num_banks_per_group, _num_rows, _num_word_cols;
 ...
 public:
  ...
  mem_controller(const sc_module_name& name, const std::string& config_file, 
    const std::string& output_dir, unsigned int controller_channel_bitwidth, 
    double controller_clk_period, unsigned int input_queue_size, unsigned int output_queue_size,
  ...
  void PrintStats();
  uint64_t AddressMapping(uint64_t addr);
