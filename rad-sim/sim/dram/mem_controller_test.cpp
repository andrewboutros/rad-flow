...
mem_controller_test::mem_controller_test(
    ...
    unsigned int addressable_word_size_bytes, double clk_period, RADSimDesignContext* radsim_design) 
    : sc_module(name) {
  ...
  for (unsigned int ch_id = 0; ch_id < _num_used_channels; ch_id++) {
    for (unsigned int cmd_id = 0; cmd_id < _num_write_cmds / _burst_size;
         cmd_id++) {
      uint64_t offset_addr = (uint64_t)std::rand() % max_offset_address;
      while (offset_addr >=
             (max_offset_address - (_burst_size * (1 << _bl_bits))))
    ...
    for (unsigned int cmd_id = 0; cmd_id < _num_read_cmds / _burst_size;
         cmd_id++) {
      uint64_t offset_addr = (uint64_t)std::rand() % max_offset_address;
      while (offset_addr >=
             (max_offset_address - (_burst_size * (1 << _bl_bits))))
...
void mem_controller_test::aw_source() {
  ...
  while (!aw_done) {
    for (unsigned int ch_id = 0; ch_id < _num_used_channels; ch_id++) {
      if (tx_interface[ch_id].awvalid.read() &&
          tx_interface[ch_id].awready.read()) {
    ...
    for (unsigned int ch_id = 0; ch_id < _num_used_channels; ch_id++) {
      aw_done =
          aw_done && (_aw_count[ch_id] == (_num_write_cmds / _burst_size));
    }
  }
  for (unsigned int ch_id = 0; ch_id < _num_channels; ch_id++) {
    tx_interface[ch_id].awvalid.write(false);
...
void mem_controller_test::w_source() {
  ...
  while (!w_done) {
    for (unsigned int ch_id = 0; ch_id < _num_used_channels; ch_id++) {
      if (tx_interface[ch_id].wvalid.read() &&
          tx_interface[ch_id].wready.read()) {
        std::cout << "[" << GetSimulationCycle(_clk_period) << "] ";
        std::cout << "Channel " << ch_id << " accepted W request ";
        std::cout << "(" << _w_count[ch_id] << ", ";
        std::cout << _wdata[ch_id][_w_count[ch_id]].to_uint() << ")"
                  << std::endl;
        _w_count[ch_id]++;
      }
      ...
      if (_w_count[ch_id] < _num_write_cmds) {
        ...
        tx_interface[ch_id].wlast.write((_w_count[ch_id] % _burst_size) ==
                                        (_burst_size - 1));
      } else {
        tx_interface[ch_id].wvalid.write(false);
...
void mem_controller_test::ar_source() {
  ...
  while (!ar_done) {
    for (unsigned int ch_id = 0; ch_id < _num_used_channels; ch_id++) {
      if (tx_interface[ch_id].arvalid.read() &&
          tx_interface[ch_id].arready.read()) {
        std::cout << "[" << GetSimulationCycle(_clk_period) << "] ";
...
void mem_controller_test::r_sink() {
  ...
  while (!r_done) {
    for (unsigned int ch_id = 0; ch_id < _num_used_channels; ch_id++) {
      if (tx_interface[ch_id].rready.read() &&
          tx_interface[ch_id].rvalid.read()) {
        std::cout << "[" << GetSimulationCycle(_clk_period) << "] ";
        std::cout << "Channel " << ch_id << " received R response "
                  << _r_count[ch_id] << std::endl;
        if (_r_count[ch_id] % _burst_size == 0)
          _read_latency[ch_id].push_back(
              GetSimulationCycle(_clk_period) -
              _read_issue_cycles[ch_id][_r_count[ch_id] / _burst_size]);
        _r_count[ch_id]++;
      }
    }
    wait();
  ...
  for (unsigned int ch_id = 0; ch_id < _num_used_channels; ch_id++) {
    double sum = 0;
    for (unsigned int cmd_id = 0; cmd_id < _read_latency[ch_id].size();
         cmd_id++) {
      sum += _read_latency[ch_id][cmd_id];
    }
    sum /= _read_latency[ch_id].size();
    std::cout << "Channel " << ch_id << ": " << int(sum) << " cycles"
              << std::endl;
  }
  sc_stop();
}
...
void mem_controller_test::b_sink() {
  ...
  while (!b_done) {
    for (unsigned int ch_id = 0; ch_id < _num_used_channels; ch_id++) {
      if (tx_interface[ch_id].bready.read() &&
          tx_interface[ch_id].bvalid.read()) {
        std::cout << "[" << GetSimulationCycle(_clk_period) << "] ";
        std::cout << "Channel " << ch_id << " received B response "
                  << _b_count[ch_id] << std::endl;
        _b_count[ch_id]++;
      }
    }
    wait();
...
mem_controller_system::mem_controller_system(const sc_module_name &name, RADSimDesignContext* radsim_design) 
    : sc_module(name) {
  ...
  dut_inst = new mem_controller("mem_controller", 0, radsim_design); 
  ...
  test_inst = new mem_controller_test(
      ...
      dut_inst->GetAddressableWordSize(), clk_period, radsim_design); 
  test_inst->clk(*clk_sig);
  test_inst->rst(rst_sig);

  for (unsigned int ch_id = 0; ch_id < num_channels; ch_id++) {
    test_to_dut[ch_id].Connect(test_inst->tx_interface[ch_id],
                               dut_inst->mem_channels[ch_id]);
  }
}
